...
public class ResponseSteps extends BaseStep {
    ...
    public void i_check_the_response_has_returned_exactly_drink(Integer numberOfCocktailResults) {
        assertThat(getCocktailSearchResponse().getCocktails().size()).as("expected %s number of cocktails/drinks in the search response", numberOfCocktailResults)
    ...
    public void i_check_the_search_response_contain_the_following_cocktails(DataTable table) {
        ...
        for (int i = 0; i < cocktailSearchResponse.getCocktails().size(); i++){
            assertThat(cocktailSearchResponse.getCocktails().get(i).getStrDrink()).as("list of returned cocktails does not matched expected list")
    ...
    private IngredientResponse getIngredientResponse() {
        return testContext.getResponse().jsonPath().getObject("ingredients[0]", IngredientResponse.class);
    }
